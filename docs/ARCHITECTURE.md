...
Ruinarch is a **Unity Mono** game. All gameplay logic ships as .NET CIL
bytecode with **full metadata intact** (type, method, field, namespace names —
verified unobfuscated across 4980 types). This is *not* a native
byte-matching project like the GTA SA decomp. It is a **source-restoration**
project: recover a clean, organized, documented, and ultimately **recompilable**
C# tree that reproduces the game's behavior.
...
- **monodis** (mono) — low-level metadata/table inspection when the decompiler
  is ambiguous.
- **Ghidra** — reserved for native code only. Not needed unless we later choose
  to study a specific `UnityPlayer.dll` internal. Low priority.

Rationale: with intact metadata, a disassembler-first (Ghidra) workflow throws
away the free names and structure that make this game 95%-recovered on the first
export. We lean on the CIL decompiler and spend human effort on the 5% the
decompiler mangles + comprehension.
...
Per-system deep dives in `docs/systems/`, driven by curiosity + by whatever
...
- Character AI: `*Behaviour` classes, `behaviourComponent`, job system
  (`JobQueueItem`, `jobComponent`), GOAP if present.
...
- Data/content: how scriptable objects + `StreamingAssets` feed the sim.
...
- **Asset extraction** (AssetStudio/AssetRipper) to recover prefabs, sprites,
  ScriptableObject data — reconstructs the *content* the code drives.
- **Native study** (Ghidra) only if a specific engine interaction needs it.

## 6. Milestone ladder ("100%" defined)

- **M0 — Baseline** ✅ target: reproducible raw export of both assemblies.
- **M1 — Compiles**: `src/Assembly-CSharp` + `firstpass` build to DLLs with 0
  errors. → push to GitHub.
- **M2 — Drop-in runs**: our DLLs replace the originals; game boots to main
  menu and starts a world.
...
- **M5 — Clean source**: names/comments restored to human-authored quality;
  tree reads like the original repo would.

"100% RE" = **M3 + M4** (functionally-equivalent buildable source + full system
docs). M5 is the polish pass.
