...
public class UIWindowBase : MonoBehaviour, IBeginDragHandler, IEventSystemHandler, IDragHandler, IEndDragHandler
{
	...
	{
		Vector2 size = m_canvasRectTransform.rect.size;
		Vector3 result;
		Vector2 vector;
		Vector2 vector2;
		...
		else
		{
			...
			vector = -Vector2.Scale(size, m_canvasRectTransform.pivot);
			vector2 = Vector2.Scale(size, Vector2.one - m_canvasRectTransform.pivot);
		}
