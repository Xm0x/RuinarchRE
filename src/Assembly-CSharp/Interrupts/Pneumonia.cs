...
public class Pneumonia : Interrupt
{
	...
	{
		if (ChanceData.RollChance(CHANCE_TYPE.Plagued_Event_Pneumonia) && interruptHolder.actor.homeSettlement != null && PlaguedEvent.HasMinimumAmountOfPlaguedVillagersForEvent(interruptHolder.actor.homeSettlement) && !interruptHolder.actor.homeSettlement.eventManager.HasActiveEvent(SETTLEMENT_EVENT.Plagued_Event) && interruptHolder.actor.homeSettlement.eventManager.CanHaveEvents())
		{
			interruptHolder.actor.homeSettlement.eventManager.AddNewActiveEvent(SETTLEMENT_EVENT.Plagued_Event);
		}
		interruptHolder.actor.Death("Pneumonia", null, null, interruptHolder.effectLog, null, null, this, isPlayerSource: true);
		return true;
	}
