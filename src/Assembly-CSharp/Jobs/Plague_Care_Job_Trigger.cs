...
public class Plague_Care_Job_Trigger : SettlementJobTrigger, NPCSettlementEventDispatcher.ITimeListener
{
	...
	{
		if (GameManager.Instance.GetHoursBasedOnTicks(GameManager.Instance.Today().tick) == 6 && !p_settlement.HasJob(JOB_TYPE.PLAGUE_CARE) && p_settlement.residents.Count((Character r) => r.traitContainer.HasTrait("Quarantined") && r.currentStructure.settlementLocation == p_settlement) >= 1)
		{
