...
public class ScriptableObjectsManager : BaseMonoBehaviour
{
	...
	[Header("Prism Events ")]
	[SerializeField]
	private PrismEventData[] _prismEvents;
	...
	public PrismEventData GetPrismEventData(PRISM_EVENT p_eventType)
	{
		for (int i = 0; i < _prismEvents.Length; i++)
		{
			PrismEventData prismEventData = _prismEvents[i];
			if (prismEventData.eventType == p_eventType)
			{
				return prismEventData;
			}
		}
		return null;
	}
