...
public class FeebleSpirit : TileObject
{
	...
	public override Type serializedData => typeof(SaveDataFeebleSpirit);

	public FeebleSpirit()
	{
		_duration = GameManager.Instance.GetTicksBasedOnHour(1);
		Initialize(TILE_OBJECT_TYPE.FEEBLE_SPIRIT, shouldAddCommonAdvertisements: false);
		base.traitContainer.AddTrait(this, "Feeble");
	...
	private void FeebleEffect()
	{
		...
		int p_value = 100;
		SkillData spellData = PlayerSkillManager.Instance.GetSpellData(PLAYER_SKILL_TYPE.FEEBLE_SPIRIT);
		...
		if (GameUtilities.RollChance(p_value))
		{
			...
			{
				possessionTarget.needsComponent.AdjustTiredness(adjustment);
			}
		}
		else
		{
