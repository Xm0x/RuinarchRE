...
public class PlayerSuccessRaidChaosOrb : PassiveSkill
{
	...
	private void OnSuccessParty(Party party)
	{
		if (party.isPlayerParty)
		{
			...
			{
				Messenger.Broadcast(PlayerSignals.CREATE_CHAOS_ORBS, party.members[0].worldPosition, Random.Range(1, 2), party.members[0].gridTileLocation.parentMap);
			}
			else
			{
				Messenger.Broadcast(PlayerSignals.CREATE_CHAOS_ORBS, party.deadMembers[0].worldPosition, Random.Range(1, 2), party.members[0].gridTileLocation.parentMap);
			}
		}
	}
}
