public class Disgust : Emotion
{
	...
	{
		if (p_target is Character target)
		{
			int num = -20;
			p_totalOpinionReduction += num;
			p_lastStrawReasonKey = _localizedResponseKey;
			p_witness.relationshipContainer.AdjustOpinion(p_witness, target, "Disgust", num, _localizedResponseKey, p_triggerOpinionChangesEffect);
		}
