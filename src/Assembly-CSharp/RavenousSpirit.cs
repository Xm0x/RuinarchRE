...
public class RavenousSpirit : TileObject
{
	...
	public override Type serializedData => typeof(SaveDataRavenousSpirit);

	public RavenousSpirit()
	{
		_duration = GameManager.Instance.GetTicksBasedOnHour(1);
		Initialize(TILE_OBJECT_TYPE.RAVENOUS_SPIRIT, shouldAddCommonAdvertisements: false);
		base.traitContainer.AddTrait(this, "Ravenous");
	...
	{
		return "Ravenous Spirit " + base.id;
	}
	...
	private void RavenousEffect()
	{
		...
		int p_value = 100;
		SkillData spellData = PlayerSkillManager.Instance.GetSpellData(PLAYER_SKILL_TYPE.RAVENOUS_SPIRIT);
		...
		if (GameUtilities.RollChance(p_value))
		{
			...
			if (possessionTarget.needsComponent.HasNeeds())
			{
				possessionTarget.needsComponent.AdjustFullness(adjustment);
			}
		}
		else
		{
