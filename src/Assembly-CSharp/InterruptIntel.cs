...
public class InterruptIntel : IIntel, IDisposable
{
	...
	{
		interrupt.SetShouldNotBeObjectPooled(state: true);
		interrupt.SetIsIntel(p_state: true);
	...
	public string GetIntelInfoRelationshipText()
	{
		...
		bool flag2 = false;
		bool flag3 = false;
		if (interruptHolder.interrupt.type == INTERRUPT.Flirt)
		{
				...
				for (int i = 0; i < character5.traitComponent.obsessedCharacterIDs.Count; i++)
				{
					...
					if (characterByPersistentID != null && characterByPersistentID != character5 && characterByPersistentID != character)
					{
						if (character2 == null)
						{
							...
							text += GetObsessedFlirtFeelingText(character2, character);
						}
				...
				for (int j = 0; j < character.traitComponent.obsessedCharacterIDs.Count; j++)
				{
					...
					if (characterByPersistentID2 != null && characterByPersistentID2 != character5 && characterByPersistentID2 != character)
					{
						if (character2 == null)
						{
							...
							text += GetObsessedFlirtFeelingText(character2, character5);
						}
						else if (character3 == null)
						{
		...
		{
			list.Clear();
			interruptHolder.PopulateReactionsToActor(list, character, target, character2, REACTION_STATUS.INFORMED);
		...
		{
			list.Clear();
			interruptHolder.PopulateReactionsToActor(list, character, target, character3, REACTION_STATUS.INFORMED);
		...
		{
			list.Clear();
			interruptHolder.PopulateReactionsToActor(list, character, target, character4, REACTION_STATUS.INFORMED);
	...
	public string GetIntelInfoBlackmailText()
	{
		...
		Character character = actor;
		if (CanBeUsedToBlackmailCharacter(character) && interruptHolder.crimeType != CRIME_TYPE.None && interruptHolder.crimeType != CRIME_TYPE.Unset)
		{
			CrimeType crimeType = CrimeManager.Instance.GetCrimeType(interruptHolder.crimeType);
	...
	private string GetObsessedFlirtFeelingText(Character p_sourceOfObsession, Character p_thirdPartyCharacter)
	{
		Dictionary<string, string> dictionary = MaccimaDictionaryPool<string, string>.Claim();
		...
		string localizedValue = LocalizationManager.Instance.GetLocalizedValue("UIStrings_Table", "Intel_Flirt_Emotion", dictionary);
		MaccimaDictionaryPool<string, string>.Release(dictionary);
		return localizedValue;
	}
	...
	public void OnIntelRemoved()
	{
		...
		if (interruptHolder.isSupposedToBeInPool)
		{
