...
public class ForlornSpiritData : SkillData
{
	...
	{
		ForlornSpirit forlornSpirit = InnerMapManager.Instance.CreateNewTileObject<ForlornSpirit>(TILE_OBJECT_TYPE.FORLORN_SPIRIT);
		forlornSpirit.SetGridTileLocation(targetTile);
		forlornSpirit.OnPlacePOI();
	...
	public override string GetBonusUIText()
	{
		if (string.IsNullOrEmpty(_bonusUIText))
		{
			...
			_bonusUIText = string.Format("{0} {1}", Utilities.ColorizeSpellTitle(LocalizationManager.Instance.GetLocalizedValue("UIStrings_Table", "Happiness_Drained") + ":"), increaseStatsPercentagePerLevel);
		}
		return _bonusUIText;
	}
	...
	public override string GetBonusLevelUpUIText()
	{
		if (string.IsNullOrEmpty(_bonusLevelUpUIText))
		{
			...
			if (base.isMaxLevel)
			{
				_bonusLevelUpUIText = string.Format("{0} {1}", Utilities.ColorizeSpellTitle(LocalizationManager.Instance.GetLocalizedValue("UIStrings_Table", "Happiness_Drained") + ":"), increaseStatsPercentagePerLevel);
			}
			else
			{
				...
				_bonusLevelUpUIText = string.Format("{0} {1} {2} {3}", Utilities.ColorizeSpellTitle(LocalizationManager.Instance.GetLocalizedValue("UIStrings_Table", "Happiness_Drained") + ":"), increaseStatsPercentagePerLevel, Utilities.UpgradeArrowIcon(), Utilities.ColorizeUpgradeText($"{increaseStatsPercentagePerLevel2}"));
			}
		}
