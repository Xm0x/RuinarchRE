...
public class SymptomsUIModel : MVCUIModel
{
	...
	private void ClickParalysisUpgrade()
	{
		onParalysisUpgradeClicked?.Invoke();
	}

	private void ClickVomitingUpgrade()
	{
		onVomitingUpgradeClicked?.Invoke();
	}

	private void ClickLethargyUpgrade()
	{
		onLethargyUpgradeClicked?.Invoke();
	}

	private void ClickSeizuresUpgrade()
	{
		onSeizuresUpgradeClicked?.Invoke();
	}

	private void ClickInsomniaUpgrade()
	{
		onInsomniaUpgradeClicked?.Invoke();
	}

	private void ClickPoisonCloudUpgrade()
	{
		onPoisonCloudUpgradeClicked?.Invoke();
	}

	private void ClickMonsterScentUpgrade()
	{
		onMonsterScentUpgradeClicked?.Invoke();
	}

	private void ClickSneezingUpgrade()
	{
		onSneezingUpgradeClicked?.Invoke();
	}

	private void ClickDepressionUpgrade()
	{
		onDepressionUpgradeClicked?.Invoke();
	}

	private void ClickHungerPangsUpgrade()
	{
		onHungerPangsUpgradeClicked?.Invoke();
	}

	private void OnHoverOverParalysis()
	{
		onParalysisHoveredOver?.Invoke(tooltipPosition);
	}

	private void OnHoverOverVomit()
	{
		onVomitingHoveredOver?.Invoke(tooltipPosition);
	}

	private void OnHoverOverLethargy()
	{
		onLethargyHoveredOver?.Invoke(tooltipPosition);
	}

	private void OnHoverOverSeizure()
	{
		onSeizuresHoveredOver?.Invoke(tooltipPosition);
	}

	private void OnHoverOverInsomnia()
	{
		onInsomniaHoveredOver?.Invoke(tooltipPosition);
	}

	private void OnHoverOverPoisonCloud()
	{
		onPoisonCloudHoveredOver?.Invoke(tooltipPosition);
	}

	private void OnHoverOverMonsterScent()
	{
		onMonsterScentHoveredOver?.Invoke(tooltipPosition);
	}

	private void OnHoverOverSneezing()
	{
		onSneezingHoveredOver?.Invoke(tooltipPosition);
	}

	private void OnHoverOverDepression()
	{
		onDepressionHoveredOver?.Invoke(tooltipPosition);
	}

	private void OnHoverOverHunger()
	{
		onHungerPangsHoveredOver?.Invoke(tooltipPosition);
	}

	private void OnHoverOutParalysis()
	{
		onParalysisHoveredOut?.Invoke();
	}

	private void OnHoverOutVomit()
	{
		onVomitingHoveredOut?.Invoke();
	}

	private void OnHoverOutLethargy()
	{
		onLethargyHoveredOut?.Invoke();
	}

	private void OnHoverOutSeizure()
	{
		onSeizuresHoveredOut?.Invoke();
	}

	private void OnHoverOutInsomnia()
	{
		onInsomniaHoveredOut?.Invoke();
	}

	private void OnHoverOutPoisonCloud()
	{
		onPoisonCloudHoveredOut?.Invoke();
	}

	private void OnHoverOutMonsterScent()
	{
		onMonsterScentHoveredOut?.Invoke();
	}

	private void OnHoverOutSneezing()
	{
		onSneezingHoveredOut?.Invoke();
	}

	private void OnHoverOutDepression()
	{
		onDepressionHoveredOut?.Invoke();
	}

	private void OnHoverOutHunger()
	{
		onHungerPangsHoveredOut?.Invoke();
	}
}
