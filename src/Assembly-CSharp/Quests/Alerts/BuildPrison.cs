...
public class BuildPrison : GameAlert
{
	private int _currentStep;

	public override Type serializedData => typeof(SaveDataBuildPrison);

	public int currentStep => _currentStep;

	public BuildPrison()
		: base(Game_Alert.Build_Prison)
	{
		_currentStep = 1;
	}

	public BuildPrison(SaveDataGameAlert p_data)
		: base(p_data, Game_Alert.Build_Prison)
	{
		SaveDataBuildPrison saveDataBuildPrison = p_data as SaveDataBuildPrison;
		_currentStep = saveDataBuildPrison.currentStep;
	}

	public override void SetAsSpawned()
	{
		SkillData skillData = PlayerSkillManager.Instance.GetSkillData(PLAYER_SKILL_TYPE.TORTURE_CHAMBERS);
	...
	protected override void SetAsCleared()
	{
		...
		Messenger.RemoveListener(UISignals.SHOW_SNATCH_VILLAGER_UI, OnShowSnatchVillagerUI);
		Messenger.RemoveListener<SkillData, int>(PlayerSkillSignals.CHARGES_UPDATED, OnChargesAdjusted);
		Messenger.RemoveListener<LocationStructure>(StructureSignals.STRUCTURE_OBJECT_PLACED, OnStructurePlacedForSpawnedAlert);
		Messenger.RemoveListener<PLAYER_SKILL_TYPE>(PlayerSkillSignals.PLAYER_GAINED_DEMONIC_STRUCTURE, OnPlayerGainedDemonicStructure);
	}
	...
	public override void OnHoverOverBookmarkItem(UIHoverPosition p_pos)
	{
		...
		else if (_currentStep == 3)
		{
			...
			dictionary2.Add("shortcutKey", InputManager.Instance.GetShortcutDisplayStringForAction(SHORTCUT_ACTION.Right_Click));
			info = GetLocalizedString(_strGameAlertType + "_Tooltip_3", dictionary2);
			MaccimaDictionaryPool<string, string>.Release(dictionary2);
			Messenger.Broadcast(UISignals.SHOW_SELECTABLE_GLOW, "Snatch Villager");
		}
	...
	public override void OnHoverOutBookmarkItem()
	{
		...
		{
			Messenger.Broadcast(UISignals.HIDE_SELECTABLE_GLOW, "Snatch Villager");
		}
	...
	private void SetActiveStepAs3()
	{
		...
		base.bookmarkEventDispatcher.ExecuteBookmarkChangedNameOrElementsEvent(this);
		Messenger.RemoveListener<SkillData>(PlayerSkillSignals.ON_EXECUTE_PLAYER_SKILL, OnStructurePlacedForStep2);
		Messenger.AddListener(UISignals.SHOW_SNATCH_VILLAGER_UI, OnShowSnatchVillagerUI);
	}

	private void OnPlayerGainedDemonicStructure(PLAYER_SKILL_TYPE p_spell)
	{
		if (p_spell == PLAYER_SKILL_TYPE.TORTURE_CHAMBERS)
		{
			AlertValid();
		}
	}
	...
	{
		if (p_skillData.type == PLAYER_SKILL_TYPE.TORTURE_CHAMBERS)
		{
	...
	private void OnShowSnatchVillagerUI()
	{
		RemoveBookmark();
	}
	...
	{
		if (p_structure.structureType == STRUCTURE_TYPE.TORTURE_CHAMBERS)
		{
			SetAsCleared();
