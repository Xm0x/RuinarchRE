...
public class MagicAcademy : ManMadeStructure
{
	private LocationGridTile _unseizedObjectFrom;

	private const int Needed_Dummies = 2;

	private const int Needed_Water_Basins = 3;

	private const int Needed_Desks = 3;

	private const int Needed_Books = 2;

	public MagicAcademy(Region location)
		: base(STRUCTURE_TYPE.MAGIC_ACADEMY, location)
	{
		SetMaxHPAndReset(6000);
	}

	public MagicAcademy(Region location, SaveDataManMadeStructure data)
		: base(location, data)
	{
		SetMaxHP(6000);
	}
	...
	{
		if (!base.hasBeenDestroyed && removedObject.mapObjectState == MAP_OBJECT_STATE.BUILT && !HasEnoughOfObject(removedObject) && (removedObject is TrainingDummy || removedObject is Desk || removedObject is Book || removedObject is WaterBasin))
		{
			...
			goapPlanJob.SetStillApplicableChecker("IsMagicAcademyCraftStillApplicable");
	...
	public bool HasEnoughOfObject(TileObject p_tileObject)
	{
		...
		{
			TILE_OBJECT_TYPE.TRAINING_DUMMY => GetNumberOfBuiltTileObjects(p_tileObject.tileObjectType) >= 2, 
			TILE_OBJECT_TYPE.WATER_BASIN => GetNumberOfBuiltTileObjects(p_tileObject.tileObjectType) >= 3, 
			TILE_OBJECT_TYPE.DESK => GetNumberOfBuiltTileObjects(p_tileObject.tileObjectType) >= 3, 
			TILE_OBJECT_TYPE.BOOK => GetNumberOfBuiltTileObjects(p_tileObject.tileObjectType) >= 2, 
			_ => true, 
		};
	}
}
