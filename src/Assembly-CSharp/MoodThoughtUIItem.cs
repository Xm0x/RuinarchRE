...
{
	[SerializeField]
	private TextMeshProUGUI txtThought;

	[SerializeField]
	private TextMeshProUGUI txtMoodModificationAmount;
	...
	[SerializeField]
	private EventLabel eventLabelThoughts;

	private GameDate _expiryDate;

	private Action<GameDate> _onHoverOverItem;

	private Action _onHoverOutItem;
	...
	public void SetItemDetails(string p_thought, int p_amount, GameDate p_expiryDate, Action<GameDate> p_onHoverOverItem, Action p_onHoverOutItem)
	{
		...
		if (p_amount > 0)
		{
			...
			txtMoodModificationAmount.text = "<color=\"green\">+" + p_amount + "</color>";
		}
		else if (p_amount < 0)
		{
			imgBG.sprite = spriteNegative;
			txtMoodModificationAmount.text = "<color=\"red\">" + p_amount + "</color>";
		}
		else
		{
