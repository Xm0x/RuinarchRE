...
public class CriticalBreakUIView : MVCUIView
{
	...
	public CriticalBreakUIModel UIModel => _baseAssetModel as CriticalBreakUIModel;

	public static void Create(Canvas p_canvas, CriticalBreakUIModel p_assets, Action<CriticalBreakUIView> p_onCreate)
	{
		CriticalBreakUIView criticalBreakUIView = new GameObject(typeof(CriticalBreakUIView).ToString()).AddComponent<CriticalBreakUIView>();
		CriticalBreakUIModel assets = UnityEngine.Object.Instantiate(p_assets);
		criticalBreakUIView.Init(p_canvas, assets);
		p_onCreate?.Invoke(criticalBreakUIView);
	}
	...
	{
		CriticalBreakUIModel uIModel = UIModel;
	...
	public void PlayShowAnimation()
	{
		...
		{
			CriticalBreakItemUI criticalBreakItemUI = UIModel.items[i];
			_showSequence.Join(criticalBreakItemUI.PrepareAnimation().SetDelay((float)i / 5f));
		}
		_showSequence.Play();
	}
	...
	public void PlayItemsAnimation()
	{
		...
		{
			CriticalBreakItemUI criticalBreakItemUI = UIModel.items[i];
			_itemsSequence.Join(criticalBreakItemUI.PrepareAnimation().SetDelay((float)i / 5f));
		}
		_itemsSequence.Play();
	}
