...
{
	public BuyStone()
		: base(INTERACTION_TYPE.BUY_STONE)
	{
	...
	{
		AddExpectedEffect(InteractionManager.Instance.GetGoapEffectData(GOAP_EFFECT_CONDITION.TAKE_POI, "Stone Pile", p_isKeyANumber: false, GOAP_EFFECT_TARGET.ACTOR));
		AddExpectedEffect(InteractionManager.Instance.GetGoapEffectData(GOAP_EFFECT_CONDITION.HAS_POI, "Stone Pile", p_isKeyANumber: false, GOAP_EFFECT_TARGET.ACTOR));
		AddExpectedEffect(InteractionManager.Instance.GetGoapEffectData(GOAP_EFFECT_CONDITION.BUY_OBJECT, "Stone Pile", p_isKeyANumber: false, GOAP_EFFECT_TARGET.ACTOR));
	}

	public override void Perform(ActualGoapNode goapNode)
	{
		base.Perform(goapNode);
		if (goapNode.poiTarget is StonePile && goapNode.poiTarget.gridTileLocation != null && goapNode.poiTarget.gridTileLocation.structure is ManMadeStructure manMadeStructure && manMadeStructure.CanPurchaseFromHere(goapNode.actor, out var needsToPay, out var _))
		{
	...
	public override GoapActionInvalidity IsInvalid(ActualGoapNode node)
	{
		...
		if (!goapActionInvalidity.isInvalid && node.poiTarget is StonePile stonePile && node.poiTarget.gridTileLocation != null && node.poiTarget.gridTileLocation.structure is ManMadeStructure manMadeStructure)
		{
			if (manMadeStructure.CanPurchaseFromHere(node.actor, out var needsToPay, out var _))
			{
				...
				else if (stonePile.resourceInPile < GetResourceAmount(node))
				{
					goapActionInvalidity.isInvalid = true;
	...
	protected override bool AreRequirementsSatisfied(Character actor, IPointOfInterest poiTarget, OtherData[] otherData, JobQueueItem job)
	{
		if (base.AreRequirementsSatisfied(actor, poiTarget, otherData, job))
		{
			...
			if (poiTarget is StonePile stonePile && poiTarget.gridTileLocation != null && poiTarget.gridTileLocation.structure is ManMadeStructure manMadeStructure && manMadeStructure.CanPurchaseFromHere(actor, out var _, out var _))
			{
				if (stonePile.resourceInPile < GetResourceAmount(otherData, job, poiTarget))
				{
					return false;
				}
				return true;
	...
	private void TakeStone(ActualGoapNode goapNode)
	{
		...
		StonePile stonePile = goapNode.target as StonePile;
		if (stonePile.resourceInPile <= resourceAmount)
		{
			goapNode.actor.PickUpItem(stonePile);
			return;
		}
		StonePile stonePile2 = InnerMapManager.Instance.CreateNewTileObject<StonePile>(stonePile.tileObjectType);
		stonePile2.SetResourceInPile(resourceAmount);
		goapNode.actor.PickUpItem(stonePile2);
		stonePile.AdjustResourceInPile(-resourceAmount);
	}
